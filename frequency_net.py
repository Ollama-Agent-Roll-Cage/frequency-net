...
class FrequencyNet(nn.Module):
    ...
    """
    def __init__(self, input_channels=3, fourier_scale=10.0, fast_mode=False):
        super().__init__()
        ...
        # Lightweight network for fast mode
        ...
        # Cache for frequency domain masks
        self.register_buffer('cached_luma_mask', None)
        self.register_buffer('cached_chroma_mask', None)
    
    def _create_frequency_masks(self, h, w, device):
        """Create and cache frequency domain masks"""
        if (self.cached_luma_mask is None or 
            self.cached_luma_mask.shape[-2:] != (h, w//2 + 1)):
            
            y_freq = torch.linspace(-1, 1, h, device=device)
            x_freq = torch.linspace(-1, 1, w//2 + 1, device=device)
            grid_y, grid_x = torch.meshgrid(y_freq, x_freq, indexing='ij')
            dist = torch.sqrt(grid_y**2 + grid_x**2)
            
            self.cached_luma_mask = (torch.exp(-dist * self.fourier_scale) + 0.3).unsqueeze(0).unsqueeze(0)
            self.cached_chroma_mask = (torch.exp(-dist * (self.fourier_scale * 0.8)) + 0.4).unsqueeze(0).unsqueeze(0)
    
    def forward(self, x):
        try:
            x = x.float()  # Ensure float32 precision
            
            if self.fast_mode:
                # Fast mode: Simple spatial processing with minimal FFT
                spatial_out = self.fast_network(x)
                b, c, h, w = spatial_out.shape
                
                # Only process luminance in frequency domain
                luma = 0.299 * spatial_out[:,0:1] + 0.587 * spatial_out[:,1:2] + 0.114 * spatial_out[:,2:3]
                
                # Convert to double precision for FFT
                luma = luma.double()
                freq = torch.fft.rfft2(luma, dim=(-2, -1), norm='ortho')
                
                # Use cached mask or create if needed
                self._create_frequency_masks(h, w, x.device)
                freq_filtered = freq * self.cached_luma_mask.double()
                freq_out = torch.fft.irfft2(freq_filtered, s=(h, w), dim=(-2, -1), norm='ortho')
                
                # Convert back to float32
                freq_out = freq_out.float()
                
                # Blend only luminance
                output = spatial_out.clone()
                output[:,0:1] = 0.7 * spatial_out[:,0:1] + 0.3 * freq_out
                ...
                # Create/get frequency masks
                self._create_frequency_masks(h, w, x.device)
                
                # Process each channel with double precision FFT
                for ch in range(c):
                    channel = spatial_out[:, ch:ch+1].double()
                    freq = torch.fft.rfft2(channel, dim=(-2, -1), norm='ortho')
                    mask = self.cached_luma_mask if ch == 0 else self.cached_chroma_mask
                    freq_filtered = freq * mask.double()
                    freq_out = torch.fft.irfft2(freq_filtered, s=(h, w), dim=(-2, -1), norm='ortho')
                    freq_outputs.append(freq_out.float())
                
                freq_out = torch.cat(freq_outputs, dim=1)
                return 0.7 * spatial_out + 0.3 * freq_out
                
        except Exception as e:
            print(f"Error in FrequencyNet forward: {str(e)}")
            return x
